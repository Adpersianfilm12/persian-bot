...
def send_welcome(message):
  ...
  if unjoined:
    ...
    markup.add(InlineKeyboardButton("✅ بررسی عضویت / دریافت فیلم", callback_data=f"check_sub_{start_param}"))
    bot.reply_to(message, "⚠️ **محدودیت دسترسی!**\n\nبرای دریافت فیلم، لطفاً ابتدا در کانال‌های زیر عضو شوید:", reply_markup=markup, parse_mode="Markdown")
    return
  handle_movie_delivery(chat_id, start_param)

def handle_movie_delivery(chat_id, start_param):
  if not start_param:
    bot.send_message(chat_id, "✅ عضویت شما تایید شد! حالا می‌توانید از طریق لینک‌های داخل کانال فیلم‌ها را دریافت کنید.")
    return
  try:
    status_msg = bot.send_message(chat_id, "🎬 در حال دریافت فیلم از سرور آرشیو...")
    copied_msg = bot.copy_message(chat_id=chat_id, from_chat_id=ARCHIVE_CHANNEL_ID, message_id=int(start_param))
    video_msg_id = copied_msg.message_id
    try:
      bot.delete_message(chat_id, status_msg.message_id)
    except Exception:
      pass
    warning_text = "⚠️ **توجه مهم:**\n📁 این فایل به زودی پاک خواهد شد، لطفاً آن را در پیام‌های ذخیره‌شده (Saved Messages) فوروارد یا ذخیره کنید! ⏳"
    redownload_text = "📥 در صورت پاک شدن، از دکمه‌ی زیر استفاده کنید 👇"
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("♻️ دانلود مجدد فیلم", callback_data=f"redownload_{start_param}"))
    ...
    bot.send_message(chat_id, "❌ خطایی در ارسال فیلم رخ داد یا لینک فیلم نامعتبر است.")

@bot.callback_query_handler(func=lambda call: call.data.startswith("check_sub_"))
def callback_check_sub(call):
  user_id = call.from_user.id
  start_param = call.data.split("_")[2] if len(call.data.split("_")) > 2 else ""
  unjoined = get_unjoined_channels(user_id)
  if unjoined:
    bot.answer_callback_query(call.id, "❌ شما هنوز در تمام کانال‌های معرفی‌شده عضو نشده‌اید!", show_alert=True)
  else:
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("redownload_"))
...
@bot.callback_query_handler(func=lambda call: call.data in ["admin_stats", "admin_broadcast"])
def admin_buttons(call):
  ...
  if call.data == "admin_stats":
    ...
    bot.edit_message_text(f"📊 **آمار ربات شما:**\n\n👥 تعداد کل کاربران ربات: `{total_users}` نفر", call.message.chat.id, call.message.message_id, parse_mode="Markdown")
  elif call.data == "admin_broadcast":
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id, "📢 لطفاً پیام خود را برای ارسال همگانی به تمام کاربران بفرستید:")
...
def process_broadcast(message):
  ...
  for user_id in users_database:
    try:
      ...
      failed += 1
  bot.edit_message_text(f"✅ **ارسال همگانی به پایان رسید!**\n\n📤 ارسال شده: {success}\n❌ ناموفق (بلاک کرده‌اند): {failed}", message.chat.id, status_msg.message_id, parse_mode="Markdown")
