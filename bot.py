...
# دیتابیس موقت برای نگهداری کانال‌ها و کاربران
bot_data = {
    "channels": CHANNELS.copy(),
    "users": set()
}

# تابع بررسی عضویت در کانال‌های جوین اجباری
...
async def start_command(client, message: Message):
    ...
    # بررسی اینکه آیا کاربری روی لینک فیلم کلیک کرده است یا خیر
...
# تابع ارسال فیلم با قابلیت حذف خودکار بعد از 20 ثانیه و دکمه دانلود مجدد
async def send_movie_file(client, chat_id, file_code):
    # پیام اصلی فیلم (که بعد از 20 ثانیه پاک می‌شود)
    sent_msg = await client.send_message(
        chat_id, 
        f"🎬 **فیلم درخواستی شما** (کد: {file_code})\n\n⏳ این فایل پس از ۲۰ ثانیه پاک خواهد شد!"
    )
    
    # متن هشدار و دکمه دانلود مجدد
    ...
    # تایمر 20 ثانیه‌ای برای حذف خودکار فایل و هشدار بالا
    await asyncio.sleep(20)
    try:
        await sent_msg.delete()
        await msg_warning.delete()
        # پیام دارای دکمه دانلود مجدد باقی می‌ماند
    except Exception:
        pass

# مدیریت درخواست دانلود مجدد
...
async def panel_callbacks(client, callback_query):
    ...
    if data == "p_stats":
        ...
        await callback_query.message.reply("📢 برای ارسال پیام همگانی، متن خود را به صورت ریپلی روی این پیام یا ارسال مستقیم بفرستید.")
    
    elif data == "p_add_ch":
        await callback_query.message.reply("➕ لطفاً آیدی کانال جدید را با علامت @ بفرستید (مثلاً @new_channel):")
    
    elif data == "p_del_ch":
        channels_str = "\n".join(bot_data["channels"])
        await callback_query.message.reply(f"➖ کانال‌های فعلی جوین اجباری:\n{channels_str}\n\nآیدی کانالی که می‌خواهید حذف کنید را ارسال کنید.")
