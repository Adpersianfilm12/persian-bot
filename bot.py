...
# تابع پس‌زمینه برای پاک کردن فقط فیلم و پیام هشدار بعد از ۱۰ ثانیه
async def delete_messages_task(bot: Bot, chat_id: int, msg_ids: list):
...
# تابع ارسال فیلم از کانال آرشیو (فقط فیلم و هشدار بعد از ۱۰ ثانیه پاک میشن، دکمه دانلود میمونه)
async def send_archive_file(message: Message, bot: Bot, file_id_or_msg_id: str):
  try:
    ...
    # ارسال پیام دانلود مجدد (این پیام پاک نخواهد شد تا کاربر همیشه بتونه دانلود کنه)
    ...
    # فقط فیلم و پیام هشدار بعد از ۱۰ ثانیه حذف میشن
    asyncio.create_task(
        delete_messages_task(
            bot,
            message.chat.id,
            [forwarded_msg.message_id, warning_msg.message_id],
        )
    )
...
async def callback_redownload(callback: CallbackQuery, bot: Bot):
  ...
  await callback.answer("در حال ارسال مجدد فیلم...")
